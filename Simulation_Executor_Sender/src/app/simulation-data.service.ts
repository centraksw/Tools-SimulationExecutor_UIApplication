...
export class SimulationDataService {
  ...
  getQuotes(): Observable<any> {
    ...
    this.socket.on('redisMessage', (res) => { //DEVICEDATA
  ...
  publishMessage(channel: string, message: string): Observable<any> {
    const apiUrl = 'http://localhost:3000/publish';
    console.log("redisRes:" + apiUrl);
    return this.http.post(apiUrl, { channel, message });
  }
